...
- **Manage backend dependencies with uv.** Treat `backend/pyproject.toml` as the backend dependency declaration. From `backend/`, use `uv add <package>` for runtime dependencies, `uv add --dev <package>` for development dependencies, and `uv remove` to remove them. Run `mise run be:sync` (or `uv sync`) to sync the env, and run Python through `mise run be:*` or `uv run`. Do not use pip, conda, or poetry for the backend unless asked. Commit `backend/uv.lock`. The repo-root `pyproject.toml` is the setuptools package for the `catan_bot` CLI — install with `mise run bot:sync` and run `mise run bot:choose-move`.
