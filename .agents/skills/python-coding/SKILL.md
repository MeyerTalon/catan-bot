...
- **Manage dependencies with uv, in one workspace.** The repo-root `pyproject.toml` (the `catan_bot` package) is a uv workspace with `backend/` as a member: one `uv.lock` and one `.venv`, both at the root. Backend deps go in `backend/pyproject.toml` (`uv add <package>` / `uv add --dev <package>` / `uv remove` from `backend/`); bot deps go in the root `pyproject.toml` (`uv add` from the root). Sync with `mise run sync` (`uv sync --frozen --all-packages --all-groups`), run Python through `mise run be:*` / `mise run bot:*` or `uv run` from the package's directory, and commit the root `uv.lock`. Never create a second venv or lockfile under `backend/`; do not use pip, conda, or poetry unless asked.
- **Unit tests for critical functions.** When writing or changing a function whose failure would break API, auth, persistence, or move-selection behavior, add a lightweight unit test that confirms its core behavior. Tests must run in seconds: tiny inputs, no network, no real database, no Ollama. Use `mise run be:test` (or `uv run pytest` from `backend/`; plain test functions with asserts — no test classes unless grouping helps). Put backend tests in `backend/tests/test_<module>.py`. Skip tests for trivial glue or one-off scripts.
- **Follow existing patterns.** Mirror nearby modules for FastAPI routers, SQLAlchemy models, Pydantic schemas, CRUD, services, and `catan_bot` models/CLI before introducing a new approach.
