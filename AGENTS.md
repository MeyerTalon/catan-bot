...
- Game engine layout: `engine/src/{models.py, actions.py, board.py, engine.py}` with tests in `engine/tests/`; same ruff/mypy setup as the backend, configured in `engine/pyproject.toml` (`mise run ge:check`). Keep it free of I/O so it stays importable from the backend and any bot
